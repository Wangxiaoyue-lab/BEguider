...
def GetParams(dataset):
    if dataset == "ABE7.10-NGG":
        ...
        sys.exit()
    effpath = os.path.join(script_dir, effpath)
    proppath = os.path.join(script_dir, proppath)
...
def process_dir(BaseEditor, seq_path, outpath):
    ...
    if platform.system() == "Windows":
        ...
        propout = (
            ...
            + "\\outputs\\"
            + name
            + "_Predicted_Editing_Proportion_for"
            + "_"
            + BaseEditor
            + ".csv"
        )
        ...
        name = seq_path.split("/")[-1].split(".")[0]
        effout = (
            cwd + "/outputs/" + name + "_designed_sgRNA_for" + "_" + BaseEditor + ".csv"
        )
        propout = (
            cwd
            + "/outputs/"
...
def get_offtarget(fname, tempdir):
    ...
    if platform.system() == "Windows":
        ...
        cmd = script_dir + "\\win-cas-offinder.exe " + inpath + " C" + " " + outpath
        # print('Windows:', cmd)
    elif platform.system() == "Linux":
        cmd = script_dir + "/linux-cas-offinder " + inpath + " C" + " " + outpath
        cmd2 = "chmod u+x " + script_dir + "/linux-cas-offinder"
        chmo = Popen(args=cmd2, shell=True)
        chmo.wait()
        # print('Linux:', cmd)
    elif platform.system() == "Darwin":
        cmd = script_dir + "/mac-cas-offinder " + inpath + " C" + " " + outpath
        cmd2 = "chmod u+x" + script_dir + " /mac-cas-offinder"
        chmo = Popen(args=cmd2, shell=True)
        chmo.wait()
        # print('MacOS:', cmd)
    else:
...
def get_prop_result(dataset, y_pred):
    ...
    for i, dfdata in locate.groupby(by="row"):
        ...
        temp["Predicted-Editing-Outcomes"] = (
            temp["pos1-3"] + temp["pos4-8"] + temp["pos9-20"]
        )

        if refseq in temp["Predicted-Editing-Outcomes"]:
            ref_prop = temp[temp["Predicted-Editing-Outcomes"] == refseq][
                "Pred-Proportion"
            ].to_list()[0]
...
def do_predict(baseeditor, genes_info, model_path):
    ...
    propdata["Pred-Proportion"] = propdata["Pred-Proportion"].apply(
        lambda x: round(x, 4)
    )
    # print('effdata:',effdata.shape)
