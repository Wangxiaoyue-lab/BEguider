...
def get_xml_filename(genome, chrom, startpos, endpos, cache_dir):
    current_filename = "{0}/{1}_{2}_{3}_{4}.xml".format(
        cache_dir, genome, chrom, str(startpos), str(endpos)
    )
...
def fetch_dna_coordinates(genome, chrom, startpos, endpos, cache_dir):
    current_filename = get_dna_coordinates_xmlfile(
        genome, chrom, startpos, endpos, cache_dir
    )
    ...
    seq = re.sub(
        "\s",
        "",
        xmldata.childNodes[1].childNodes[1].childNodes[1].childNodes[0].data.upper(),
    )
...
if __name__ == "__main__":
    seq = fetch_dna_coordinates("hg38", "16", 88842824, 88842847, "./")
    print(seq)
