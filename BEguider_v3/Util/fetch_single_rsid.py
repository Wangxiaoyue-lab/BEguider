...
def get_snp(snp_id, temp_dir):
	...
	res["genomic_flanking"] = \
				fetch_dna_coordinates("hg38", res["chromosome"],
											res["coordinate"] - 20, res["coordinate"] + 20, temp_dir)
		#print(res["chromosome"], res["coordinate"], res["genomic_flanking"])
	return [res["chromosome"], res["coordinate"], res["genomic_flanking"]]
	...
	for k, v in mutations.items():
		try:
			...
			for feature in response_item.features:
				if feature.type == "CDS":
					#print('feature location:',feature.location.start, feature.location.end)
			...
			if not (all_snps[-1]["5UTR"] or all_snps[-1]["3UTR"]):
				...
				all_snps[-1]["CDS_flanking"] = mrna[max(0,snp_idx-25):min(snp_idx+25, len(mrna))]
			else:
